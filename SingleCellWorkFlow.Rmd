---
title: "Single Cell Workflow ABACBS 2017"
author: "Shian Su"
...
Single cell RNA sequencing (scRNA-seq) is a powerful technique that is used to investigate the gene expression profiles of individual cells. This is in contrast to classic bulk tissue RNA sequencing whereby the gene expression profiles represent the average profiles of a large number of cells.

Single cell sequencing allows for higher resolution analysis, allowing for a range of new types of discoveries not possible with bulk sequencing. However certain characteristics make single cell sequencing data slightly more difficult to work with than bulk rna seq.

There are usually large numbers of cells sequenced, which generally results in at least some degraded cells that should be excluded from analysis. In general the gene count numbers are low and in addition there are gene drop-outs whereby many genes have 0 counts. These have adverse effects on existing modelling techniques.
...
We can visualise the results of the comparison using an interactive Glimma plot. This shows MD (now MA) plot along with a 

```{r}
keep <- dge$samples$cell_type == "B_Lymph" | dge$samples$cell_type == "T_Lymph"
glMDPlot(
    lrt,
    counts = cpm(dge$counts, log=TRUE)[, keep],
    anno = data.frame(Symbol = rownames(lrt$table)),
    groups = dge$samples$cell_type[keep],
...
We conclude this workshop here, but with the `SingleCellExperiment` and `DGEList` objects constructed a variety of further analyses can be performed.
