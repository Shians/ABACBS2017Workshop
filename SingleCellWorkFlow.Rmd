...
It's often informative to check the most highly expressed genes, this can be easily compared with reference datasets and provide additonal diagnostic insight. Below we see high expression of genes related to red blood cells.
...
We filter by the number of samples a gene can be detected in and the average counts of the gene within samples where it is expressed. Taking the average only within expressed samples is important as not to remove imformative genes that are well expressed only in a small population. Here we have set the thresholds
...
We can see that the SC3 assigned clustering matches up with the known cell types. These asssigned cluster indices help with downstream differential expression analysis.
...
In contrast to a standard `edgeR` pipeline we do not run `calcNormFactors` as we have already calculated size factors from `scran` which were transferred when running `convertTo`. The `scran` method for size factors is more robust to gene dropout.
...
For the contrast we have a few choices, most commonly we could perform pairwise comparisons between our clusters, alternatively one could compare one cluster to the average of the remaining clusters. Here for demonstration purposes we simply compare clusters 4 and 5 of the `SC3` clusters. 
